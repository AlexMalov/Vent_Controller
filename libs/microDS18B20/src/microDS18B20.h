...
// ====================== CLASS ======================
template <uint8_t DS_PIN, uint8_t DS_WRT_PIN, uint8_t *DS_ADDR = (uint8_t*)nullptr, uint8_t DS_AM = 1, bool DS_PGM = 0>
class MicroDS18B20 {
public:
    MicroDS18B20() {
        pinMode(DS_PIN, INPUT_PULLUP);
        pinMode(DS_WRT_PIN, INPUT_PULLUP);
        digitalWrite(DS_WRT_PIN, LOW);
    }
    bool useMQTT[DS_AM];
    bool mqttRegistered[DS_AM];      // канал зарегисрирован на MQTT
    char* mqtt_topic_state[DS_AM];

    // Установить разрешение термометра 9-12 бит
    void setResolution(uint8_t res, uint8_t idx = 0) {
        if (!oneWire_reset(DS_PIN, DS_WRT_PIN)) return;         // Проверка присутствия
        addressRoutine(idx);                        // Процедура адресации
        oneWire_write(0x4E, DS_WRT_PIN);                // Запись RAM
        oneWire_write(0xFF, DS_WRT_PIN);                // Максимум в верхний регистр тревоги
        oneWire_write(0x00, DS_WRT_PIN);                // Минимум в верхний регистр тревоги
        oneWire_write(((constrain(res, 9, 12) - 9) << 5) | 0x1F, DS_WRT_PIN); // Запись конфигурации разрешения
    }
    ...
    bool readAddress(uint8_t *addr) {
        if (!oneWire_reset(DS_PIN, DS_WRT_PIN)) return 0;       // Проверка присутствия
        oneWire_write(0x33, DS_WRT_PIN);                // Запрос адреса
        uint16_t sum = 0;                           // контрольная сумма
        uint8_t crc = 0;                            // обнуляем crc
        for (uint8_t i = 0; i < 8; i++) {           // Прочитать 8 байт адреса
            addr[i] = oneWire_read(DS_PIN, DS_WRT_PIN);         // Записать в массив
    ...
    void requestTemp(uint8_t idx = 0) {
        ...
        if (!oneWire_reset(DS_PIN, DS_WRT_PIN)) return;         // Проверка присутствия
        addressRoutine(idx);                        // Процедура адресации
        oneWire_write(0x44, DS_WRT_PIN);                // Запросить преобразование
    }
    ...
    bool readTemp(uint8_t idx = 0) {
        if (!oneWire_reset(DS_PIN, DS_WRT_PIN)) return 0;       // датчик оффлайн
        addressRoutine(idx);                   		// Процедура адресации
        oneWire_write(0xBE, DS_WRT_PIN);                // Запросить температуру
        uint8_t crc = 0;                            // обнуляем crc
        int16_t temp;                               // переменная для расчёта температуры
        uint16_t sum = 0;                           // контрольная сумма
        for (uint8_t i = 0; i < 9; i++) {           // Считать RAM
            uint8_t data = oneWire_read(DS_PIN, DS_WRT_PIN);    // Прочитать данные
            sum += data;
            #if (DS_CHECK_CRC == true)
    ...
    bool online(uint8_t idx = 0) {
        if (_addr != nullptr) {
            if (!oneWire_reset(DS_PIN, DS_WRT_PIN)) return 0;
            addressRoutine(idx);
            oneWire_write(0xBE, DS_WRT_PIN);
            uint16_t sum = 0;
            for (uint8_t i = 0; i < 5; i++) sum += oneWire_read(DS_PIN, DS_WRT_PIN);
            return ((sum != 0x4FB)&&(sum != 0));
        } else return oneWire_reset(DS_PIN, DS_WRT_PIN);
    }
...
private:
    ...
    void addressRoutine(uint8_t idx) {              // Процедура адресации
        if (_addr != nullptr) {                   // Адрес определен?
            oneWire_write(0x55, DS_WRT_PIN);            // Говорим термометрам слушать адрес
            for (uint8_t i = 0; i < 8; i++) {
                if (DS_PGM) oneWire_write(pgm_read_byte(&_addr[i + idx * 8]), DS_WRT_PIN);
                else oneWire_write(_addr[i + idx * 8], DS_WRT_PIN);
            }
        } else oneWire_write(0xCC, DS_WRT_PIN);         // Адреса нет - пропускаем адресацию на линии
    }
